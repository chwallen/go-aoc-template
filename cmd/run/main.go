...
func runAllDaysForYear(year int) {
	...
	for i, day := range days {
		...
		for _, dayPart := range day {
			...
			fmt.Printf(
				"part %d: %s (duration: %d.%d ms)\n",
				dayPart.Part, result, elapsed.Milliseconds(),
...
func run(dayPart RunnerInput) AdventOfCodeResult {
	...
	return AdventOfCodeResult{Result: output, TimeElapsed: duration}
}
